...
Local web server for the FLAIR NPU demo site. Runs the CPU (PyTorch) and NPU
inference pipelines concurrently over the sample dataset, streams live
progress to a browser dashboard, and reports the comparative speedup +
accuracy once both finish.
...
Real inference (npu/run_dataset_inference.py + npu/cpu_baseline.py) requires
torch, numpy and ml_dtypes, and for the NPU side must run from the WSL IRON
environment with prebuilt xclbins (see README: build once with
`make -f Makefile.encoder && make -f Makefile.decoder && make -f Makefile.batch`
in npu/, then run this server with --skip-build, which is the default).
...
clearly labeled as mock data in the API response.

Usage:
    python3 webdemo/server.py --skip-build            # real run, prebuilt xclbins
...
# npu/run_dataset_inference.py and npu/cpu_baseline.py import each other with
# bare (non-package) names, e.g. `from run_dataset_inference import ...` --
# they expect npu/ itself on sys.path (true when run directly as a script,
# since Python adds a script's own dir to sys.path[0]). Add both dirs here so
# the same modules work when imported from this server instead.
sys.path.insert(0, str(_REPO))
sys.path.insert(0, str(_NPU_DIR))

DEFAULT_NPZ = str(_REPO / "data" / "processed" / "preprocessed.npz")
DEFAULT_SEQ_LEN = 10
DEFAULT_LIMIT = 200
    ...
    def cpu_worker():
        ...
        try:
            if mock:
                ...
                from cpu_baseline import run_cpu_pipeline
    ...
    def npu_worker():
        ...
        try:
            if mock:
                ...
                from run_dataset_inference import run_npu_pipeline
